...
def train(cfg, local_rank, distributed, logger):
    ...
    if cfg.MODEL.META_ARCHITECTURE == 'SimrelRCNN':
        if cfg.MODEL.SimrelRCNN.FREEZE_BACKBONE:
    ...
    for iteration, (images, targets, _) in enumerate(train_data_loader, start_iter):
        ...
        #    elif len(target.get_field("labels").long()) < 1 or \
        #            len(torch.where(target.get_field("relation").long() > 0)[0]) < 1:
        #        continue
        ...
        #        print('-->name:', name, '-->grad_requirs:', parms.requires_grad, '--weight', torch.mean(parms.data), ' -->grad_value_mean:', torch.mean(parms.grad), ' -->grad_value_norm:', parms.grad.data.norm())
        #    else:
        ...
        if cfg.SOLVER.TO_VAL and iteration % cfg.SOLVER.VAL_PERIOD == 0:
            logger.info("{}/{} iter Start validating".format(iteration, cfg.SOLVER.VAL_PERIOD))
...
def run_val(cfg, model, val_data_loaders, distributed, logger):
    ...
    if dataset_names[0].find("OI") >= 0 or dataset_names[0].find("oi") >= 0 \
      or dataset_names[0].find("open") >= 0 or dataset_names[0].find("Open") >= 0:
        val_values = []
