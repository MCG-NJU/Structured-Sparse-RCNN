...
def load_state_dict(model, loaded_state_dict, load_mapping):
    ...
    load_ckpt_rel(model, model_state_dict)

def load_ckpt_rel(model, ckpt):
    """Load checkpoint"""
    for k, v in model.state_dict().items():
        if k in ckpt:
            param = torch.from_numpy(np.asarray(ckpt[k].data.cpu()))
            if v.shape == param.shape:
                v.copy_(param)
            else:
                print('[Loaded net not complete] Parameter[{}] Size Mismatch. Param size: {} -x-> {}'.format(k, v.shape, param.shape))