...
class VGDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, split, img_dir, roidb_file, dict_file, image_file, transforms=None,
        ...
        if image_file.find('vrd') >= 0:
            num_val_im = min(num_val_im, 500)
...
def load_image_filenames(img_dir, image_file):
    ...
    for i, img in enumerate(im_data):
        ...
        if os.path.exists(filename):
            ...
            img_info.append(img)
    if image_file.find('vg') >= 0: ###
        assert len(fns) == 108073, '{}, {}'.format(len(fns), len(img_info))
...
def load_graphs(roidb_file, split, num_im, num_val_im, filter_empty_rels, filter_non_overlap):
    ...
    #print(roi_h5['img_to_first_box'][:])
    #print(roi_h5['img_to_first_rel'][:])
    #print(num_val_im, num_im)
    #print(data_split)
    ...
    #print(image_index, num_val_im, num_val_im)
    #assert False
