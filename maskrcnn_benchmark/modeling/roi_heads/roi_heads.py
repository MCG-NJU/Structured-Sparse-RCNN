...
class CombinedROIHeads(torch.nn.ModuleDict):
    ...
    def forward(self, features, proposals, targets=None, logger=None, ori_images=None):
        losses = {}
        x, detections, loss_box = self.box(features, proposals, targets, ori_images)
        
        if self.cfg.MODEL.SimrelRCNN.TRAIN_OBJ or not self.cfg.MODEL.RELATION_ON:
            # During the relationship training stage, the bbox_proposal_network should be fixed, and no loss. 
            losses.update(loss_box)
            return x, detections, losses ###!!! to-do
...
def build_roi_heads(cfg, in_channels):
    ...
    if not cfg.MODEL.SimrelRCNN.TRAIN_OBJ:
