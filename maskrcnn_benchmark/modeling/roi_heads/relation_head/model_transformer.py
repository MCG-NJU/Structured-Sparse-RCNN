...
class TransformerContext(nn.Module):
    def __init__(self, config, obj_classes, rel_classes, in_channels):
        ...
        self.register_buffer("untreated_obj_feat", torch.zeros(self.hidden_dim))
    ...
    def forward(self, roi_features, proposals, rel_pair_idxs=None, logger=None, ctx_average=False):
        ...
        if ctx_average and self.effect_analysis and (not self.training):
            obj_feats = self.untreated_obj_feat.view(1, -1).expand(batch_size, -1)
            prim_obj_feats = self.context_obj(obj_pre_rep, num_objs)
        else:
        ...
        if self.mode == 'predcls':
            ...
            if ctx_average and self.effect_analysis and (not self.training):
                ...
                tmp = self.untreated_edg_feat
                avg_obj_embed2, avg_roi_feat = tmp[:self.embed_dim], tmp[self.embed_dim:]
                avg_roi_feat = avg_roi_feat.view(1, -1).expand(batch_size, -1)
                avg_obj_embed2 = avg_obj_embed2.view(1, -1).expand(batch_size, -1)
                edge_pre_rep = cat((avg_roi_feat, obj_feats, avg_obj_embed2), dim=-1)
            else:
                edge_pre_rep = cat((roi_features, obj_feats, obj_embed2), dim=-1)
        else:
            if ctx_average and self.effect_analysis and (not self.training):
                obj_dists = self.out_obj(prim_obj_feats)
            else:
        ...
        self.untreated_obj_feat = self.moving_average(self.untreated_obj_feat, obj_feats)
        self.untreated_edg_feat = \
            self.moving_average(self.untreated_edg_feat, cat((obj_embed2, roi_features), -1))
