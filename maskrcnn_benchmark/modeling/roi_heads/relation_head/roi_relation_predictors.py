...
class TransformerPredictor(nn.Module):
    def __init__(self, config, in_channels):
        ...
        self.vg_logits_adjustment = config.MODEL.SimrelRCNN.REL_LOGITS_ADJUSTMENT
        self.logit_adj_tau = config.MODEL.SimrelRCNN.LOGIT_ADJ_TAU
        vg_rel_freq = config.MODEL.ROI_RELATION_HEAD.REL_PROP
        if self.num_rel_cls > len(vg_rel_freq):
            vg_rel_freq = [1.] + vg_rel_freq
    ...
    def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger=None):
        obj_dists, rel_dists, pair_pred, add_losses = \
            self.forward_v0(proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger=logger)
        if self.effect_analysis and (not self.training):
            _, rel_dists_average, _, _ = \
                self.forward_v0(proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, alien_pair_pred = None ,ctx_average=True)
            rel_ans = []
            for i, j in zip(rel_dists, rel_dists_average):
                rel_ans.append(i - j)
            rel_dists = rel_ans
        return obj_dists, rel_dists, add_losses
        
    def forward_v0(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, alien_pair_pred=None, logger=None, ctx_average=False):
        """
        Returns:
        ...
        if self.attribute_on:
            ...
            obj_dists, obj_preds, edge_ctx = self.context_layer(roi_features, proposals, logger=logger, ctx_average=ctx_average)
        ...
        if alien_pair_pred is not None:
            pair_pred = alien_pair_pred
        ...
        ctx_val = self.ctx_compress(prod_rep)
        vis_val = self.rel_compress(visual_rep)
        rel_dists = vis_val + ctx_val
                
        # use frequence bias
        if self.use_bias:
            freq_val = self.freq_bias.index_with_labels(pair_pred)
            rel_dists = rel_dists + freq_val

        if self.effect_type == 'AL':
            rel_dists = rel_dists - self.logit_adj_tau * self.vg_rel_freq
        ...
        if self.training and self.effect_analysis:
            ...
            add_losses['auxiliary_ctx'] = F.cross_entropy(ctx_val, rel_labels)
            if self.use_vision:
                add_losses['auxiliary_vis'] = F.cross_entropy(vis_val, rel_labels)
            if self.use_bias:
                add_losses['auxiliary_frq'] = F.cross_entropy(freq_val, rel_labels)

        if self.attribute_on:
            att_dists = att_dists.split(num_objs, dim=0)
            return (obj_dists, att_dists), rel_dists, pair_pred, add_losses
        else:
            return obj_dists, rel_dists, pair_pred, add_losses
...
class CausalAnalysisPredictor(nn.Module):
    def __init__(self, config, in_channels):
        ...
        self.register_buffer("average_rel_logits", torch.zeros(self.num_rel_cls))
        self.vg_logits_adjustment = config.MODEL.SimrelRCNN.REL_LOGITS_ADJUSTMENT
        self.logit_adj_tau = config.MODEL.SimrelRCNN.LOGIT_ADJ_TAU
        vg_rel_freq = config.MODEL.ROI_RELATION_HEAD.REL_PROP
        if self.num_rel_cls > len(vg_rel_freq):
    ...
    def pair_feature_generate(self, roi_features, proposals, rel_pair_idxs, num_objs, obj_boxs, logger, ctx_average=False):
        # encode context infomation
        if self.context_layer_name == 'transformer':
            binary_preds = None
            obj_dists, obj_preds, edge_ctx = \
                self.context_layer(roi_features, proposals, rel_pair_idxs, logger, ctx_average=ctx_average)
        else:
            obj_dists, obj_preds, edge_ctx, binary_preds = \
                self.context_layer(roi_features, proposals, rel_pair_idxs, logger, ctx_average=ctx_average)
    ...
    def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger=None):
        ...
        if self.training:
            cur_rel_logits = rel_dists.view(-1, rel_dists.shape[-1])
            self.average_rel_logits = self.moving_average(self.average_rel_logits, cur_rel_logits)
        ...
        if self.training:
            ...
            if self.effect_type == 'TDE':   # TDE of CTX
                ...
                if self.effect_type == 'AL':
                    #rel_dists = self.calculate_logits(union_features, post_ctx_rep, pair_obj_probs) - 0.5 * self.average_rel_logits
                    rel_dists = self.calculate_logits(union_features, post_ctx_rep, pair_obj_probs) - self.logit_adj_tau * self.vg_rel_freq
            rel_dist_list = rel_dists.split(num_rels, dim=0)
